...
/*exports.smsg = (conn, m, hasParent) => {
     ...
     */
     ...
     m.reply = async (text, chatId, options) => {
        ...
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: `${ucpn}`, body: wm, sourceUrl: `https://wa.me/${global.nomorown}`, thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
